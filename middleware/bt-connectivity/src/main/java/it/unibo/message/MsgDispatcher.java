...
public class MsgDispatcher {
     ...
     */
    public Msg poll() {
        Msg pendingMsg = pendingMessages.poll();
        if (pendingMsg != null)
    ...
    public JsonObject removeWaitingMessage(long id) {
        ...
        Iterator<Msg> it2 = this.ackMessages.iterator();
        while (it2.hasNext()) {
            JsonObject msg = it2.next().getJsonObject();
            if (msg.getLong("id").equals(id)) {
                it2.remove();
                return msg;
            }
    ...
    public boolean containsMsg(long id) {
        List<Msg> copyList = new ArrayList<>(this.ackMessages);
        Iterator<Msg> it = copyList.iterator();
        while (it.hasNext()) {
            JsonObject msg = it.next().getJsonObject();
            if (msg.getLong("id").equals(id)) {
                return true;
